...
\item DateGenerationRule [Optional]: Controls how the expiry (and for OI futures, the start) date is derived from the
  expiry month. Accepted values are listed below. If not given this field defaults to `IMM'.
  \begin{itemize}
  \item For MM futures the following rules are allowed:
    \begin{itemize}
    \item \texttt{IMM}: Expiry is the 3rd Wednesday of the expiry month.
    \item \texttt{IMMAUD} (alias \texttt{SecondThursday}): Expiry is the 2nd Thursday of the expiry month. Used for
      AUD-BBSW-3M futures. \texttt{SecondThursday} is accepted as a backward-compatible alias and will be serialised
      as \texttt{IMMAUD}.
    \item \texttt{IMMNZD}: Expiry is the first Wednesday on or after the 9th day of the expiry month (pure weekday
      arithmetic, no holiday calendar adjustment). Used for NZD bank-bill futures.
    \item \texttt{IMMCAD}: Expiry is two business days (using a combined UK and Canada holiday calendar) before the
      3rd Wednesday of the expiry month. Used for CAD BAX / CORRA futures.
    \item \texttt{IMMEUR}: Expiry is two EUREX business days before the 3rd Wednesday of the expiry month. Used for
      EUR-EURIBOR-3M futures.
    \end{itemize}
  \item For an overnight index future `IMM' means that the start date of the future is set to the 3rd Wednesday of the
    contract month and the end date is set to the 3rd Wednesday of the contract month plus the future tenor. The setting
    `IMM' applies to SOFR-3M futures for example. `FirstDayOfMonth' on the other hand means that the start date of the
    future is set to the first day in the contract month and the end date is set to the
    first day of the month lying $n$ months after the start date's month where $n$ is the number of months of the
    future's underlying tenor. The setting `FirstDayOfMonth' applies to SOFR-1M futures for example. For `FirstDayOfMonth'
    the start and end date are adjusted to the next business day w.r.t. roll convention `following' and the overnight
    index calendar - or if given the calendar given in the field `Calendar'. This tenor is
    derived from the market quote, see \ref{ss:market_data_oi_index_future_prices}.
  \end{itemize}
\item Calendar: Only applicable to overnight index futures with date generation rule `FirstDayOfMonth'. Overwrites the
    calendar used to adjust the start and end date of the future. Optional, if not given, the overnight index publishing
    calendar will be used to adjust the future start and end date.
...
\item OvernightIndexTenor [Optional]: This field is mandatory for par sensitivity calculation when using overnight index futures.
  Specifies the tenor for the future (e.g., 3M for SOFR futures). When specified, the tenor must match the tenor in the market data quotes during curve
  building, otherwise an error will be raised. For example, for SOFR 3M futures, this should be set to 3M. This field can be left blank if no par conversion
  is needed (i.e., for curve building only).
\end{itemize}

Listings \ref{lst:future_conventions_euribor_3m}, \ref{lst:future_conventions_aud_bbsw_3m},
\ref{lst:future_conventions_nzd_bkbm_3m}, \ref{lst:future_conventions_cad_corra_3m},
\ref{lst:future_conventions_sofr_3m}, \ref{lst:future_conventions_sofr_1m} show examples for Euribor-3M,
AUD-BBSW-3M, NZD-BKBM-3M, CAD-CORRA-3M, SOFR-3M and SOFR-1M future conventions.
...
<Future>
  ...
  <DateGenerationRule>IMMEUR</DateGenerationRule>
</Future>
\end{minted}
\caption{Euribor 3M MM Future conventions (IMMEUR -- two EUREX business days before the third Wednesday)}
...
<Future>
  ...
  <DateGenerationRule>IMMAUD</DateGenerationRule>
</Future>
\end{minted}
\caption{AUD BBSW 3M MM Future conventions (IMMAUD -- second Thursday expiry; \texttt{SecondThursday} is a
...
\begin{minted}[fontsize=\footnotesize]{xml}
<Future>
  <Id>NZD-BKBM-3M-FUTURES</Id>
  <Index>NZD-BKBM-3M</Index>
  <DateGenerationRule>IMMNZD</DateGenerationRule>
</Future>
\end{minted}
\caption{NZD BKBM 3M MM Future conventions (IMMNZD -- first Wednesday on or after the 9th of the expiry month)}
\label{lst:future_conventions_nzd_bkbm_3m}
\end{listing}
...
\begin{minted}[fontsize=\footnotesize]{xml}
<Future>
  <Id>CAD-CORRA-3M-FUTURES</Id>
  <Index>CAD-CORRA-3M</Index>
  <DateGenerationRule>IMMCAD</DateGenerationRule>
</Future>
\end{minted}
\caption{CAD CORRA 3M MM Future conventions (IMMCAD -- two UK/Canada business days before the third Wednesday)}
\label{lst:future_conventions_cad_corra_3m}
\end{listing}
...
\ref{lst:fra_conventions}. The underlying index name is given in the
\lstinline!Index! node. For example, this could be EUR-EURIBOR-6M, CHF-LIBOR-6M etc.

The optional \lstinline!EndDateConvention! node controls how the FRA maturity (end) date is calculated:
\begin{itemize}
\item \lstinline!FromSpot! (default) --- the end date is obtained by a single date roll from the spot date, applying the combined duration of the forward start period and the index tenor.
\item \lstinline!FromStart! --- the end date is obtained in two steps: first, the start date is computed by rolling from spot by the forward start period; second, the end date is computed by rolling from that start date by the index tenor. When date adjustments (e.g., end-of-month, business day conventions) apply unevenly across intermediate dates, these two approaches may yield different results.
\end{itemize}
If the \lstinline!EndDateConvention! node is omitted, \lstinline!FromSpot! is assumed for backward compatibility.
...
<FRA>
  <Id> </Id>
  <Index> </Index>
  <EndDateConvention> </EndDateConvention>
</FRA>
\end{minted}
...
<TenorBasisSwap>
  ...
  <SubPeriodsCouponType> </SubPeriodsCouponType>
  <PayIsAveraged> </PayIsAveraged>
  <RecIsAveraged> </RecIsAveraged>
</TenorBasisSwap>
\end{minted}
...
\item PayIsAveraged [Optional]: \emph{True} if the pay leg coupon is averaged and \emph{False} if it is compounded.
\item RecIsAveraged [Optional]: \emph{True} if the receive leg coupon is averaged and \emph{False} if it is compounded.
\end{itemize}
...
%- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
\subsubsection{BMA / SIFMA Basis Swap Conventions}
%- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
...
\item TradingCalendar [Optional]: Trading center calendar used when adjusting valid business days for the FX swap rate helper, joined with the above AdvanceCalendar. If
not provided, it defaults to a calendar with no holidays.
\end{itemize}
...
<CrossCurrencyBasis>
  ...
  <SpreadTenor> </SpreadTenor>
  <FlatTenor> </FlatTenor>
  <FlatPaymentLag> </FlatPaymentLag>
  <SpreadPaymentLag> </SpreadPaymentLag>
  <FlatPaymentLag> </FlatPaymentLag>
  <!-- for OIS only -->
  <SpreadIncludeSpread> </SpreadIncludeSpread>
  <SpreadLookback> </SpreadLookback>
  <SpreadFixingDays> </SpreadFixingDays>
  <SpreadRateCutoff> </SpreadRateCutoff>
  <SpreadIsAveraged> </SpreadIsAveraged>
  <SpreadObservationShift> </SpreadObservationShift>
  ...
  <FlatObservationShift> </FlatObservationShift>
</CrossCurrencyBasis>
\end{minted}
...
\item SpreadObservationShift [Optional]: Only relevant if spread leg is OIS, allowable values are true, false, defaults to true if not given
\item FlatIncludeSpread [Optional]: Only relevant if flat leg is OIS, allowable values are true, false, defaults to false if not given
\item FlatLookback [Optional]: Only relevant if flat leg is OIS, allowable values are 0D, 1D, ..., defaults to 0D if not given
\item FlatFixingDays [Optional]: Only relevant if flat leg is OIS, allowable values are 0, 1, 2, ..., defaults to 0 if not given
\item FlatRateCutoff [Optional]: Only relevant if flat leg is OIS, allowable values are 0, 1, 2, ..., defaults to 0 if not given
\item FlatIsAveraged [Optional]: Only relevant if flat leg is OIS, allowable values are true, false, defaults to false if not given
\item FlatObservationShift [Optional]: Only relevant if flat leg is OIS, allowable values are true, false, defaults to true if not given
\end{itemize}

\subsubsection{Cross Currency Fix Float Swap Conventions}
A node with name \lstinline!CrossCurrencyFixFloat! is used to store conventions for cross currency fixed versus float swap quotes.
The structure of this node is shown in Listing \ref{lst:xccy_fix_float_conventions}.
...
<CrossCurrencyFixFloat>
  ...
  <EOM> </EOM>
  <IsResettable> </IsResettable>
  <FloatIndexIsResettable> </FloatIndexIsResettable>
  <!-- for OIS only -->
  <IncludeSpread> </IncludeSpread>
  <Lookback> </Lookback>
  ...
  <ObservationShift> </ObservationShift>
</CrossCurrencyFixFloat>
\end{minted}
\caption{Cross currency fix float swap conventions}
\label{lst:xccy_fix_float_conventions}
\end{listing}

The meanings of the various elements in this node are as follows:
\begin{itemize}
\item \lstinline!SettlementDays!: The number of business days to the start of the cross currency basis swap.
\item \lstinline!SettlementCalendar!: The business day calendar(s) for both legs
    It is also used to arrive at the settlement date using the SettlementDays above.
\item \lstinline!SettlementConvention!: The roll convention for both legs.
\item \lstinline!FixedCurrency!: The currency of the fixed leg.
\item \lstinline!FixedFrequency!: The frequency of the fixed leg payments.
\item \lstinline!FixedConvention!: The business day roll convention for the fixed leg.
\item \lstinline!FixedDayCounter!: The day count convention for the fixed leg.
\item \lstinline!Index!: The name of the index on the floating leg.
\item \lstinline!EOM! [Optional]:
    \lstinline!true! if the end of month convention is to be used when generating the schedule on both legs, 
    and \lstinline!false! if not. If not provided, it defaults to \lstinline!false!.
\item \lstinline!IsResettable! [Optional]: \lstinline!true! if the swap is mark-to-market resetting, 
    and \lstinline!false! otherwise. If not provided, it defaults to \lstinline!false!.
\item \lstinline!FloatIndexIsResettable! [Optional]: If the swap is mark-to-market resetting, 
    then this field indicates which leg's notional resets.
    It is \lstinline!true! if it is the notional on the floating leg that resets, 
    and \lstinline!false! if it is the notional on the fixed leg that resets.
    If not provided, it defaults to \lstinline!true!.
\end{itemize}

The following fields are only relevant if the floating leg is an overnight indexed leg:
\begin{itemize}
\item \lstinline!IncludeSpread! [Optional]: If the leg is compounding,
    this field indicates whether or not to include the spread in the compounding.
    The allowable values are \lstinline!true! or \lstinline!false!.
    The default to \lstinline!false! if not given.
\item \lstinline!Lookback! [Optional]: The lookback period for the overnight indexed leg.
    If provided, it must be a period that is a non-negative number of days.
    The allowable values are \lstinline!0D!, \lstinline!1D! etc.
    It defaults to \lstinline!0D! if not given.
\item \lstinline!FixingDays! [Optional]: The fixing lag to be used for the overnight index.
    If provided, it must be a non-negative integer.
    If not provided, the natural fixing lag of the overnight index is used.
\item \lstinline!RateCutoff! [Optional]: The number of rate cut-off days on the overnight leg.
    If provided, it must be a non-negative integer.
    If not provided, it defaults to \lstinline!0! i.e. no rate cut-off.
\item \lstinline!IsAveraged! [Optional]: This flag indicates whether the overnight leg is averaged or compounded.
    The allowable values are \lstinline!true! or \lstinline!false!.
    It defaults to \lstinline!false! if not given, i.e. assumes a compounded overnight leg.
\item \lstinline!ObservationShift! [Optional]: If a non-zero lookback is given,
    this flag indicates if the there is an observation shift or not.
    The allowable values are \lstinline!true! or \lstinline!false!.
    It defaults to \lstinline!true! if not given.
\end{itemize}
...
\item \lstinline!DeliveryLocation! [Optional]: Delivery location or delivery basis identifier for the commodity contract (for example, \lstinline!XCEC!, \lstinline!XLME!, \lstinline!PM!). When provided, this value is used by FRTB commodity delta CRIF generation as \lstinline!Label2!.
...
\item \lstinline!DeliveryLocation! [Optional]: Delivery location or delivery basis identifier for the commodity contract. When provided, this value is used by FRTB commodity delta CRIF generation as \lstinline!Label2! for commodity sensitivities. This can have a small impact on FRTB commodity capital via the commodity basis correlation term when two records have different delivery locations.
...
\input{parameterisation/conventions_intradaypowercurves}
