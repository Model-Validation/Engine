...
While interpolation method and variable can be combined in an arbitrary way, only a subset of these combinations are
usually recommended to use as summarized in table \ref{tab:recommende_interp_var_methods}.
...
\begin{tabular}{|l|p{9cm}|}
    ...
    MonotonicLogCubicSpline & Monotonic cubic spline interpolation with zero second derivative at left and right \\ \hline
...
\begin{tabular}{|l|l|}
    \hline
    {\bfseries Variable} & {\bfseries Method} \\
    \hline
    Zero        & Linear \\
                & NaturalCubic \\
                & FinancialCubic \\
    ...
    Discount    & LogLinear \\
                ...
                & LogFinancialCubic \\
                & LogCubicSpline \\
                & MonotnicLogCubicSpline \\
                & DefaultLogMixedLinearCubic \\
                & MonotonicLogMixedLinearCubic \\
                & KrugerLogMixedLinearCubic \\
                & LogMixedLinearCubicNaturalSpline \\ \hline
  \end{tabular}
  \caption{Recommended combinations of interpolation variable and method.}
  \label{tab:recommende_interp_var_methods}
\end{table}
...
\item instruments from the end of the second segment with pillar date strictly later than $d_3 - 10$, where $d_3$ is the
  minimum pillar date of instruments in the third segment, will be removed
\end{itemize}
...
\item InflationIndexCurves [Optional]: for each inflation index that is required by one of the bonds to which the curve
  is fitted (e.g.\ inflation-linked bonds such as TIPS with a CPI leg), a mapping to the inflation curve used to project
  that index must be provided. Each \lstinline!InflationIndexCurve! child node carries an \lstinline!inflationIndex!
  attribute naming the index (e.g.\ \emph{USCPI}) and its value is the CurveId of the inflation curve used to project
  the bond's index-linked cashflows. The projected cashflows are scaled to nominal terms by the bond's current inflation
  factor before the nominal discount curve is fitted, so the resulting fitted curve is a nominal yield curve. If no bond
  in the basket references an inflation index, this node may be omitted.
    ...
    <YieldCurve>
      ...
      <Segments>
        <FittedBond>
          ...
          <!-- mapping of inflation curves used in the bonds from which the curve is built
               (only needed for inflation-linked bonds, e.g. TIPS) -->
          <InflationIndexCurves>
            <InflationIndexCurve inflationIndex="USCPI">USCPI_ZC_Swaps</InflationIndexCurve>
          </InflationIndexCurves>
...
\begin{itemize}
  ...
  \item InflationIndexCurves [Optional]: for each inflation index that is required by one of the bonds to which the curve
    ...
    that index must be provided. Each \lstinline!InflationIndexCurve! child node carries an \lstinline!inflationIndex!
    attribute naming the index (e.g.\ \emph{USCPI}) and its value is the CurveId of the inflation curve used to project
    the bond's index-linked cashflows. The bond's yield is computed from the price scaled to nominal terms by the bond's
    current inflation factor. If no bond in the basket references an inflation index, this node may be omitted.
  \item ExtrapolateFlat: if true, the parametric curve is extrapolated flat in the instantaneous forward rate before the
    first and after the last maturity of the bonds in the calibration basket. This avoids unrealistic rates at the short
    end or for long maturities in the resulting curve.
  \end{itemize}
